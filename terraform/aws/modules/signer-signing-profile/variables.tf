...
variable "platform" {
  type        = string
  description = "the platform to be used for signing - either 'lambda' or 'container'"
  default     = "lambda"
  sensitive   = false

  validation {
    condition     = contains(["lambda", "container"], var.platform)
    error_message = "valid values for platform are 'lambda' and 'container'"
  }
}
